...
export default function InfiniteScroll({ 
  ...
  const duration = speed === 'fast' ? '20s' : speed === 'normal' ? '20s' : '40s';
  ...
  return (
    <div className="relative w-full overflow-hidden bg-background">
      ...
      <div 
        ...
        style={{ animation: `${direction ? 'scroll' : 'scroll-reverse'} ${duration} linear infinite` }}
      >
        <div className="flex shrink-0 gap-2 items-center justify-around min-w-full">
          {children}
        </div>
        <div className="flex shrink-0 gap-2 items-center justify-around min-w-full" aria-hidden="true">
          {children}
        </div>
      </div>
    </div>
